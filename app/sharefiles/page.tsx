      ...
      <div className="min-h-screen flex items-center justify-center p-4">
        <div className="container max-w-4xl">
          ...
          <div className="space-y-6">
            ...
            {isConnected ? (
              <Card className="backdrop-blur-xl rounded-2xl border border-white/20 bg-white/5 shadow-xl overflow-hidden">
                <CardContent className=" space-y-6 p-6 ">
                  <div
                    {...getRootProps()}
                    className={`border-2 border-dashed rounded-2xl p-10 text-center cursor-pointer transition-colors ${isDragActive ? "border-purple-400 bg-purple-400/10" : "border-white/30 hover:border-white/50"
                      }`}
                  >
                    <input {...getInputProps()} />
            ...
            {/* History section */}
            <Card className="backdrop-blur-xl rounded-2xl border border-white/20 bg-white/5 shadow-xl overflow-hidden">
              <CardHeader className="pb-4 flex flex-row justify-between items-center px-4 md:px-6">
                <CardTitle className="text-white text-lg md:text-xl">Recent Transfers</CardTitle>
                <Button
                  variant="ghost"
                  onClick={clearHistory}
                  className="rounded-xl h-9 px-3 bg-white/10 hover:bg-white/20 text-white transition-all duration-200 hover:scale-105"
                ...
                <ScrollArea className="h-[400px] px-2">
                  {transferHistory.length === 0 ? (
                    <div className="flex flex-col items-center justify-center h-full py-16">
                      <FileX className="h-16 w-16 text-white/30 mb-3" />
                      <p className="text-white/50 text-base">No Recent Transfers</p>
                    </div>
                  ) : (
                    <div className="p-2 md:p-4 space-y-3">
                      {transferHistory.map((record, index) => (
                        <div
                          ...
                          className="p-4 rounded-xl bg-white/5 hover:bg-white/10 transition-all duration-200 border border-white/10 hover:border-white/20 hover:shadow-lg"
                        >
                          <div className="flex flex-col gap-3">
                            {/* File info row */}
                            <div className="flex items-start justify-between gap-3">
                              <div className="flex items-start flex-1 min-w-0">
                                <div className="bg-white/10 p-2.5 rounded-lg mr-3 flex-shrink-0">
                                  {getFileIcon(record.fileType)}
                                </div>
                                <div className="flex-grow min-w-0">
                                  <p className="text-white text-sm md:text-base font-medium truncate">{record.fileName}</p>
                                  <div className="flex flex-wrap text-white/50 text-xs md:text-sm mt-1.5 gap-x-2 gap-y-1">
                                    <span className="font-medium">{(record.fileSize / 1024).toFixed(2)} KB</span>
                                    <span className="hidden sm:inline">•</span>
                                    <span className="capitalize px-2 py-0.5 bg-white/10 rounded-md text-xs">
                                      {record.direction}
                                    </span>
                                    <span className="hidden sm:inline">•</span>
                              ...
                              <div className="flex flex-wrap gap-2 flex-shrink-0">
                                {(record.preview === "text" ||
                                  (record.preview &&
                                    ...
                                    <Button
                                      size="sm"
                                      variant="ghost"
                                      className="h-8 px-3 rounded-lg bg-purple-500/20 hover:bg-purple-500/30 text-white text-xs font-medium transition-all duration-200 hover:scale-105"
                                ...
                                <Button
                                  size="sm"
                                  variant="ghost"
                                  className="h-8 px-3 rounded-lg bg-blue-500/20 hover:bg-blue-500/30 text-white text-xs font-medium transition-all duration-200 hover:scale-105"
                                ...
                                <Button
                                  size="sm"
                                  variant="ghost"
                                  className="h-8 w-8 p-0 rounded-full bg-red-500/20 hover:bg-red-500/30 text-white transition-all duration-200 hover:scale-110"
                            ...
                            {/* Preview for images */}
                            {record.preview && record.preview !== "text" && record.preview !== "blob-reference" && record.fileType.startsWith("image/") && (
